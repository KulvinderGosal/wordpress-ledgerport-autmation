...
- Name: disconnected-state.spec.ts >> Disconnected State — Dashboard >> LP-DIS-010 | "SYNC HEALTH" label is present
- Location: tests/disconnected-state.spec.ts:112:7
...
Locator: getByText('SYNC HEALTH')
Expected: visible
Error: strict mode violation: getByText('SYNC HEALTH') resolved to 2 elements:
    1) <p class="mt-1 text-sm text-muted-foreground">Sync health, recent activity, and what needs your…</p> aka getByText('Sync health, recent activity')
    2) <p class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Sync health</p> aka getByText('Sync health', { exact: true })

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText('SYNC HEALTH')
