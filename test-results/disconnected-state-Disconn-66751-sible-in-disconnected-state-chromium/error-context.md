...
- Name: disconnected-state.spec.ts >> Disconnected State — Dashboard >> LP-DIS-011 | "View error log →" link is still visible in disconnected state
- Location: tests/disconnected-state.spec.ts:116:7
...
Locator: getByRole('link', { name: /view error log/i })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('link', { name: /view error log/i })
