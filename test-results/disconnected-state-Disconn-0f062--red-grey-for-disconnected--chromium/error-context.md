...
- Name: disconnected-state.spec.ts >> Disconnected State — Dashboard >> LP-DIS-042 | QuickBooks status dot is NOT green (is red/grey for disconnected)
- Location: tests/disconnected-state.spec.ts:263:7
...
Locator: locator('.connections-widget, [class*="connections"]').locator('[class*="disconnected"], [class*="error"], [class*="inactive"]').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.connections-widget, [class*="connections"]').locator('[class*="disconnected"], [class*="error"], [class*="inactive"]').first()
