...
- Name: disconnected-state.spec.ts >> Disconnected State — Dashboard >> LP-DIS-047 | Auto sync shows "Manual" when disconnected (not "Automatic")
- Location: tests/disconnected-state.spec.ts:287:7
...
Locator: getByText('Manual')
Expected: visible
Error: strict mode violation: getByText('Manual') resolved to 2 elements:
    1) <a href="admin.php?page=ledgerport-push">Manual Sync</a> aka getByRole('link', { name: 'Manual Sync' })
    2) <p class="text-sm font-medium text-foreground">Manual</p> aka getByText('Manual', { exact: true })

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText('Manual')
