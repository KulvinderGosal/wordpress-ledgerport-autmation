...
- Name: disconnected-state.spec.ts >> Disconnected State — Mappings Page >> LP-DIS-069 | No mapping table rows or product data are visible in disconnected state
- Location: tests/disconnected-state.spec.ts:417:7

# Error details

```
Error: locator.click: Error: strict mode violation: getByText('Products') resolved to 8 elements:
    1) <div class="wp-menu-name">Products</div> aka locator('#menu-posts-product').getByRole('link', { name: 'Products', exact: true })
    2) <li aria-hidden="true" class="wp-submenu-head">Products</li> aka locator('#menu-posts-product li').filter({ hasText: /^Products$/ })
    3) <a class="wp-first-item" href="edit.php?post_type=product">All Products</a> aka getByRole('link', { name: 'All Products' })
    4) <a href="admin.php?page=wc-admin&path=/analytics/products">Products</a> aka locator('#toplevel_page_wc-admin-path--analytics-overview').getByRole('link', { name: 'Products' })
    5) <p class="mt-1 text-sm text-muted-foreground">Map WooCommerce products, variations, customers, …</p> aka getByText('Map WooCommerce products,')
    6) <p class="text-sm leading-relaxed text-muted-foreground">Sync WooCommerce orders, products, and customers …</p> aka getByText('Sync WooCommerce orders,')
    7) <span class="text-sm text-foreground">Real-time sync as orders, products, and customers…</span> aka getByText('Real-time sync as orders,')
    8) <button role="tab" type="button" aria-selected="true" class="shrink-0 cursor-pointer !border-0 !bg-transparent px-4 py-5 text-sm !shadow-none transition-colors whitespace-nowrap !outline-none sm:px-5 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 font-semibold text-foreground">Products</button> aka getByRole('tab', { name: 'Products' })
