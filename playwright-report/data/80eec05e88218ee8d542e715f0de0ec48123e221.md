...
- Name: disconnected-state.spec.ts >> Disconnected State — Sidebar Navigation >> LP-DIS-098 | All 7 sidebar menu items are visible in disconnected state
- Location: tests/disconnected-state.spec.ts:627:7
...
Locator: locator('#adminmenu').getByText('Dashboard')
Expected: visible
Error: strict mode violation: locator('#adminmenu').getByText('Dashboard') resolved to 3 elements:
    1) <div class="wp-menu-name">Dashboard</div> aka locator('#menu-dashboard').getByRole('link', { name: 'Dashboard' })
    2) <li aria-hidden="true" class="wp-submenu-head">Dashboard</li> aka locator('#menu-dashboard li').filter({ hasText: 'Dashboard' })
    3) <a aria-current="page" class="wp-first-item current" href="admin.php?page=ledgerport">Dashboard</a> aka locator('#toplevel_page_ledgerport').getByRole('link', { name: 'Dashboard' })

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for locator('#adminmenu').getByText('Dashboard')
...
> 629 |     await expect(menu.getByText('Dashboard')).toBeVisible();
  ...
  681 |   });
  682 | 
  683 |   test('LP-DIS-105 | Configuration widget does not show real method/frequency settings from a prior connection', async () => {
  684 |     await goToPluginPage(page, PLUGIN_PAGES.dashboard);
  685 |     await expect(page.getByText('Sales Receipt')).not.toBeVisible();
  686 |     await expect(page.getByText('Hourly')).not.toBeVisible();
  687 |     // Should show "Not configured" instead
  688 |     await expect(page.getByText('Not configured').first()).toBeVisible();
  689 |   });
  690 | 
  691 |   test('LP-DIS-106 | Connections widget shows no "Connected" label for any service', async () => {
  692 |     await goToPluginPage(page, PLUGIN_PAGES.dashboard);
  693 |     const connectionsWidget = page.getByText('Connections').locator('..').locator('..');
  694 |     await expect(connectionsWidget.getByText('Connected')).not.toBeVisible();
  695 |   });
  696 | });
  697 | 
```