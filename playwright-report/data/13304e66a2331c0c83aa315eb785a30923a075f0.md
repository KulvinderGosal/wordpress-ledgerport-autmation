...
- Name: disconnected-state.spec.ts >> Disconnected State — Dashboard >> LP-DIS-028 | "Recent Activity" heading is visible
- Location: tests/disconnected-state.spec.ts:195:7
...
Expected: visible
Error: strict mode violation: getByText('Recent Activity') resolved to 2 elements:
    ...
    2) <h3 class="font-bold tracking-tight text-foreground text-base">Recent Activity</h3> aka getByRole('heading', { name: 'Recent Activity' })
