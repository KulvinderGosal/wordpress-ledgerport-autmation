...
- Name: disconnected-state.spec.ts >> Disconnected State — No Data Leakage >> LP-DIS-106 | Connections widget shows no "Connected" label for any service
- Location: tests/disconnected-state.spec.ts:691:7
...
Locator: getByText('Connections').locator('..').locator('..').getByText('Connected')
Expected: not visible
Error: strict mode violation: getByText('Connections').locator('..').locator('..').getByText('Connected') resolved to 2 elements:
    1) <span class="text-xs text-muted-foreground">Disconnected</span> aka getByText('Disconnected').first()
    2) <span class="text-xs text-muted-foreground">Disconnected</span> aka getByText('Disconnected').nth(1)

Call log:
  - Expect "not toBeVisible" with timeout 5000ms
  - waiting for getByText('Connections').locator('..').locator('..').getByText('Connected')
