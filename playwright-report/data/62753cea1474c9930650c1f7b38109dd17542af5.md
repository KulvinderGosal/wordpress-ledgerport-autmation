...
- Name: disconnected-state.spec.ts >> Disconnected State — Dashboard >> LP-DIS-008 | Sync Health card shows "0% success rate" when disconnected
- Location: tests/disconnected-state.spec.ts:104:7
...
Locator: getByText('0% success rate')
Expected: visible
Error: strict mode violation: getByText('0% success rate') resolved to 2 elements:
    1) <p class="mt-3 text-2xl font-semibold text-foreground">0% success rate</p> aka getByText('0% success rate', { exact: true })
    2) <p class="mt-1 text-sm text-muted-foreground">0.0% success rate</p> aka getByText('0.0% success rate')

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText('0% success rate')
