...
- Name: mappings.spec.ts >> LedgerPort — Mappings >> Products table has SKU column
- Location: tests/mappings.spec.ts:64:7
...
Locator: locator('th, td').filter({ hasText: /^SKU$/ }).first()
Expected: visible
Timeout: 30000ms
...
Call log:
  ...
  - waiting for locator('th, td').filter({ hasText: /^SKU$/ }).first()
