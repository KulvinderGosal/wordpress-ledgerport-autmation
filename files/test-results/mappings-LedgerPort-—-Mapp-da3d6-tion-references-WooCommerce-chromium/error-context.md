...
- Name: mappings.spec.ts >> LedgerPort — Mappings >> Products section references WooCommerce
- Location: tests/mappings.spec.ts:68:7
...
Locator: locator('#wpcontent').getByText(/WooCommerce/i).first()
Expected: visible
Timeout: 30000ms
...
Call log:
  ...
  - waiting for locator('#wpcontent').getByText(/WooCommerce/i).first()
