...
- Name: mappings.spec.ts >> LedgerPort — Mappings >> Products table has Name column
- Location: tests/mappings.spec.ts:60:7
...
Locator: locator('th, td').filter({ hasText: /^Name$/ }).first()
Expected: visible
Timeout: 30000ms
...
Call log:
  ...
  - waiting for locator('th, td').filter({ hasText: /^Name$/ }).first()
