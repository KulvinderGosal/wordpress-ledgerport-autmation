...
- Name: mappings.spec.ts >> LedgerPort — Mappings >> Products table has QuickBooks Product column
- Location: tests/mappings.spec.ts:72:7
...
Locator: locator('th').filter({ hasText: /QuickBooks Product/i }).first()
Expected: visible
Timeout: 30000ms
...
Call log:
  ...
  - waiting for locator('th').filter({ hasText: /QuickBooks Product/i }).first()
