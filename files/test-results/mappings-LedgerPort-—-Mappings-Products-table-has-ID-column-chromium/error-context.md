...
- Name: mappings.spec.ts >> LedgerPort — Mappings >> Products table has ID column
- Location: tests/mappings.spec.ts:56:7
...
Locator: locator('th, td').filter({ hasText: /^ID$/ }).first()
Expected: visible
Timeout: 30000ms
...
Call log:
  ...
  - waiting for locator('th, td').filter({ hasText: /^ID$/ }).first()
