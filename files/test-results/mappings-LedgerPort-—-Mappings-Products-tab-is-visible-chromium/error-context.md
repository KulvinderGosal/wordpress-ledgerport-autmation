...
- Name: mappings.spec.ts >> LedgerPort — Mappings >> Products tab is visible
- Location: tests/mappings.spec.ts:26:7
...
Locator: getByRole('tab', { name: /Products/i }).or(getByRole('button', { name: /^Products$/i }))
Expected: visible
Timeout: 30000ms
...
Call log:
  ...
  - waiting for getByRole('tab', { name: /Products/i }).or(getByRole('button', { name: /^Products$/i }))
