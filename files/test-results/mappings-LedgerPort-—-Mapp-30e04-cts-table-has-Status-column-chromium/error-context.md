...
- Name: mappings.spec.ts >> LedgerPort — Mappings >> Products table has Status column
- Location: tests/mappings.spec.ts:76:7
...
Locator: locator('th').filter({ hasText: /^Status$/i }).first()
Expected: visible
Timeout: 30000ms
...
Call log:
  ...
  - waiting for locator('th').filter({ hasText: /^Status$/i }).first()
